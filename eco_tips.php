...
$rows = $pdo->prepare('SELECT * FROM eco_tips'+$where+' ORDER BY id DESC'); // placeholder to be replaced
?>
<?php /* Build query safely */ ?>
<?php
  ...
  <div class="card span-6">
    ...
    <form method="get" class="form-grid" style="margin:6px 0 10px 0">
      ...
      <label class="input-group" style="display:flex;align-items:center;gap:6px; margin-top:2px">
        <input type="checkbox" name="show" value="all" <?= (($_GET['show'] ?? '')==='all')?'checked':''; ?>> Show inactive too
      </label>
      <?php endif; ?>
